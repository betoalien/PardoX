import ctypes
from .wrapper import lib, c_double, c_longlong, c_int32, c_size_t

class Series:
    """
    Represents an individual column in a PardoX DataFrame.
    Enables vectorized operations (Hybrid CPU/GPU arithmetic) using the Rust engine.
    """
    def __init__(self, df, col_name):
        self._df = df
        self.name = col_name

    def __repr__(self):
        # Render ASCII output by delegating to the parent DataFrame.
        # This keeps notebook output consistent by reusing frame.py logic.
        return self._df.__repr__()

    @property
    def dtype(self):
        """Returns the series data type by querying the parent DataFrame."""
        return self._df.dtypes.get(self.name, "Unknown")

    # =========================================================================
    # INSPECTION & VISUALIZATION
    # =========================================================================

    def head(self, n=5):
        """
        Returns a NEW Series with the first N rows.
        """
        # Build a sliced DataFrame and return the corresponding column as Series.
        new_df = self._df.head(n)
        return new_df[self.name]

    def tail(self, n=5):
        """
        Returns a NEW Series with the last N rows.
        """
        new_df = self._df.tail(n)
        return new_df[self.name]

    def show(self, n=10):
        """
        Prints the series to the console using Rust's formatter.
        """
        self._df.show(n)

    # =========================================================================
    # ARITHMETIC OPERATIONS (Cross-Manager Support)
    # =========================================================================

    def _validate_ops(self, other):
        """
        Validates that the operation is safe.
        Allows cross-manager arithmetic if row counts match.
        """
        if not isinstance(other, Series):
            raise TypeError(f"Operands must be PardoX Series objects. Got {type(other)}")
        
        if self._df.shape[0] != other._df.shape[0]:
            raise ValueError(f"Length mismatch: {self._df.shape[0]} vs {other._df.shape[0]}")
        
        return other

    def _wrap_result(self, res_ptr):
        """
        Wraps the resulting pointer into a Series.
        This enables operation chaining such as: (A + B) + C.
        """
        if not res_ptr:
            raise RuntimeError("Operation failed (Rust returned null pointer).")
        
        from .frame import DataFrame
        new_df = DataFrame(res_ptr)
        
        if len(new_df.columns) == 0:
             raise RuntimeError("Compute engine returned empty result schema.")
             
        # Rust returns a generated name (e.g. result_add), so select dynamically.
        return new_df[new_df.columns[0]]

    # --- Operators ---

    def __add__(self, other):
        other = self._validate_ops(other)
        res_ptr = lib.pardox_series_add(
            self._df._ptr, 
            self.name.encode('utf-8'), 
            other._df._ptr, 
            other.name.encode('utf-8')
        )
        return self._wrap_result(res_ptr)

    def __sub__(self, other):
        other = self._validate_ops(other)
        res_ptr = lib.pardox_series_sub(
            self._df._ptr, 
            self.name.encode('utf-8'), 
            other._df._ptr,
            other.name.encode('utf-8')
        )
        return self._wrap_result(res_ptr)

    def __mul__(self, other):
        other = self._validate_ops(other)
        res_ptr = lib.pardox_series_mul(
            self._df._ptr, 
            self.name.encode('utf-8'), 
            other._df._ptr,
            other.name.encode('utf-8')
        )
        return self._wrap_result(res_ptr)

    def __truediv__(self, other):
        other = self._validate_ops(other)
        res_ptr = lib.pardox_series_div(
            self._df._ptr, 
            self.name.encode('utf-8'), 
            other._df._ptr,
            other.name.encode('utf-8')
        )
        return self._wrap_result(res_ptr)

    def __mod__(self, other):
        other = self._validate_ops(other)
        res_ptr = lib.pardox_series_mod(
            self._df._ptr, 
            self.name.encode('utf-8'), 
            other._df._ptr,
            other.name.encode('utf-8')
        )
        return self._wrap_result(res_ptr)

    # =========================================================================
    # COMPARISONS (FILTERS)
    # =========================================================================
    
    def _cmp_op(self, other, op_code):
        """
        Generic comparison handler.
        op_code mapping: 0=Eq, 1=Neq, 2=Gt, 3=Gte, 4=Lt, 5=Lte
        """
        # Case A: Column vs Column
        if isinstance(other, Series):
            if self._df.shape[0] != other._df.shape[0]:
                raise ValueError("Length mismatch in comparison")
            
            res_ptr = lib.pardox_filter_compare(
                self._df._ptr, self.name.encode('utf-8'),
                other._df._ptr, other.name.encode('utf-8'),
                op_code
            )
            return self._wrap_result(res_ptr)
        
        # Case B: Column vs Scalar
        elif isinstance(other, (int, float)):
            is_float = 1 if isinstance(other, float) else 0
            val_f64 = float(other)
            val_i64 = int(other)
            
            res_ptr = lib.pardox_filter_compare_scalar(
                self._df._ptr, self.name.encode('utf-8'),
                c_double(val_f64), c_longlong(val_i64), c_int32(is_float),
                op_code
            )
            return self._wrap_result(res_ptr)
        
        else:
            raise TypeError(f"Comparison not supported for type {type(other)}")

    def __eq__(self, other): return self._cmp_op(other, 0)
    def __ne__(self, other): return self._cmp_op(other, 1)
    def __gt__(self, other): return self._cmp_op(other, 2)
    def __ge__(self, other): return self._cmp_op(other, 3)
    def __lt__(self, other): return self._cmp_op(other, 4)
    def __le__(self, other): return self._cmp_op(other, 5)

    # =========================================================================
    # AGGREGATIONS
    # =========================================================================

    def _call_agg(self, func_name):
        if not hasattr(lib, func_name):
            raise NotImplementedError(f"Aggregation {func_name} not available in Core.")
        
        func = getattr(lib, func_name)
        val = func(self._df._ptr, self.name.encode('utf-8'))
        return val

    def sum(self): return self._call_agg('pardox_agg_sum')
    def mean(self): return self._call_agg('pardox_agg_mean')
    def min(self): return self._call_agg('pardox_agg_min')
    def max(self): return self._call_agg('pardox_agg_max')
    def count(self): return int(self._call_agg('pardox_agg_count'))
    def std(self): return self._call_agg('pardox_agg_std')

    # =========================================================================
    # ML INTEGRATION: Zero-Copy NumPy Compatibility (v0.3.4)
    # =========================================================================

    def __array__(self, dtype=None):
        """
        NumPy protocol: exposes the underlying Rust f64 buffer as a ctypes array.
        This allows zero-copy usage: `numpy.array(df['col'])` wraps the Rust
        memory directly without duplicating the data.

        PardoX does NOT import numpy — this protocol is triggered BY numpy when
        it detects the `__array__` dunder method.
        """
        if not hasattr(lib, 'pardox_get_f64_buffer'):
            raise NotImplementedError(
                "Zero-copy array protocol not available in this build."
            )
        length_out = c_size_t(0)
        raw_ptr = lib.pardox_get_f64_buffer(
            self._df._ptr,
            self.name.encode('utf-8'),
            ctypes.byref(length_out),
        )
        if not raw_ptr:
            raise RuntimeError(
                f"Failed to obtain column buffer for '{self.name}'. "
                "Column may not be a numeric type (Float64/Int64)."
            )
        n = length_out.value
        # Map directly as ctypes.POINTER(ctypes.c_double) — true zero copy.
        # The pointer points into the HyperBlock's contiguous buffer; do NOT free it.
        ptr = ctypes.cast(raw_ptr, ctypes.POINTER(ctypes.c_double))
        arr = (ctypes.c_double * n).from_address(ctypes.cast(ptr, ctypes.c_void_p).value)
        return arr